class Goods {
	constructor() {
		// 获取节点
		this.cont = document.querySelector('.g8')
		// 调用方法
		this.getGoods();
	...
	async getGoods() {
		...
		data.forEach(goodsList => {
			...
			html += `<div class="box"><a href="getbuy.html?id=${goodsList.id}"><img src="${goodsList.src}" alt=""><p>${goodsList.name}</p><span class="goods_item_price" data-price-id="100004222715" style="">¥${goodsList.price}</span></a></div>`;
		});
